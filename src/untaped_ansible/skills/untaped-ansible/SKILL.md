...
- Graph JSON includes stable `edges[].id` values and `cycles`. Edge IDs are `edge:<digest>`, where the digest is derived from `(relation, source_id, target_id)`. Cycles are detected only inside the emitted depth-bounded graph, so increase `--depth` or use `--depth unlimited` when looking for longer loops.
- Malformed, templated, or wrong-shape dependency files are skipped with warnings instead of becoming repo failures. Empty files remain warning-free. Local/live graph reads surface parse skips as graph warnings; `source refresh` prints skipped files to stderr for that run and does not persist them into cached graph output.
